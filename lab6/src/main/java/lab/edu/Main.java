...
public class Main {
    ...
    CommandLineRunner initData(AppointmentRepository appointmentRepo,
        ...
        return args -> {
            ...
            // Update example: Change appointment time for first appointment
            Appointment firstAppt = appointmentRepo.findById(1L).orElseThrow();
            firstAppt.setAppointmentTime(LocalTime.of(11, 0));
            appointmentRepo.save(firstAppt);
            System.out.println("Updated Appointment: " + firstAppt);

            // Delete example: Delete second appointment
            appointmentRepo.deleteById(2L);
            System.out.println("Deleted appointment with ID 2. Remaining:");
            appointmentRepo.findAll().forEach(System.out::println);
        };
    }
}
